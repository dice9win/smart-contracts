...
nizk_line = re.compile(r"nizk:(?P<nizk_iters>\d+):(?P<iter>\d+):(?P<challenge_response>[^=]+=)")
responses = [(int(m['nizk_iters']), int(m['iter']), int.from_bytes(base64.b64decode(m['challenge_response']), "big"))
...
assert len(responses) >= 8, "Zero knowledge proof too short!"
assert all([nizk_iters == len(responses) for nizk_iters, _, _ in responses]), "Proof responses disagree on total count."
assert all([idx == iter for idx, (_, iter, _) in enumerate(responses)]), "Iters should increase sequentially."
assert all([pow(challenge_response, 0x10001, modulus) == challenge(nizk_iters, iter)
            for (nizk_iters, iter, challenge_response) in responses]), "Zero knowledge proof not valid!"
